...
export default function JsonBeautifier() {
  ...
  const handleBeautify = (indent: number) => {
    if (!input.trim()) {
      setError('Please enter or load JSON data.');
      setOutput('');
      return;
    }
    try {
  ...
  const handleFetchUrl = async () => {
    ...
    try {
      ...
      alert('Failed to fetch JSON from the provided URL. Check CORS or URL validity.');
    }
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 items-start">
        ...
        <div className="lg:col-span-5 space-y-2">
          ...
          <textarea
            ...
            placeholder={`Paste ugly or minified JSON here...\ne.g. {"name":"John","age":30}`}
        ...
        <div className="lg:col-span-2 space-y-3 pt-7 flex flex-col justify-center">
          ...
          <button
            ...
            className="w-full bg-slate-800 hover:bg-slate-900 text-white font-medium py-2 text-sm rounded-lg transition shadow-sm"
          >
            Minify / Compact
          </button>
