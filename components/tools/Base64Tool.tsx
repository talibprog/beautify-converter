...
export default function Base64Tool() {
  ...
  const handleEncode = () => {
    if (!input.trim()) {
      setError('Please enter or load some text to encode.');
      setOutput('');
      return;
    }
    try {
  ...
  const handleDecode = () => {
    if (!input.trim()) {
      setError('Please enter or load Base64 string to decode.');
      setOutput('');
      return;
    }
    try {
      ...
      {isUrlModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-xl border p-6 w-full max-w-md mx-4 space-y-4 animate-in fade-in zoom-in duration-200">
            <h3 className="text-lg font-bold text-gray-800">Load Content from URL</h3>
            <input
              type="url"
