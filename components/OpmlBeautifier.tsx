...
export default function OpmlBeautifier() {
  const [inputOpml, setInputOpml] = useState('');
  const [outputOpml, setOutputOpml] = useState('');
  ...
  const handleBeautify = () => {
    if (!inputOpml.trim()) return;

    try {
      const parser = new DOMParser();
      const xmlDoc = parser.parseFromString(inputOpml, 'text/xml');
      const parserError = xmlDoc.getElementsByTagName('parsererror');

      if (parserError.length > 0) {
        alert('Invalid OPML/XML syntax. Please check your input.');
        return;
      }

      const formatted = formatXml(inputOpml);
      setOutputOpml(formatted);
    } catch {
      alert('Error formatting OPML content.');
    }
  };
  ...
  const handleMinify = () => {
    ...
    try {
      ...
      const minified = inputOpml
        .replace(/<!--[\s\S]*?-->/g, '')
        .replace(/>\s+</g, '><')
        .replace(/\s+/g, ' ')
        .trim();

      setOutputOpml(minified);
    } catch {
      alert('Error minifying OPML content.');
    }
  };
  ...
  const handleFetchFromUrl = async () => {
    ...
    try {
      ...
      alert('Could not fetch OPML content from the provided URL.');
    } finally {
      setLoadingUrl(false);
    }
  };
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => setInputOpml((event.target?.result as string) || '');
    reader.readAsText(file);
  };

  const handleCopy = () => {
    if (!outputOpml) return;
    navigator.clipboard.writeText(outputOpml);
    setCopied(true);
  ...
  const handleDownload = () => {
    if (!outputOpml) return;
    const blob = new Blob([outputOpml], { type: 'text/x-opml' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'formatted.opml';
    a.click();
    ...
    <div className="p-6 bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 max-w-6xl mx-auto relative">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 items-start">
        ...
        <div className="lg:col-span-5 flex flex-col">
          ...
          <textarea
            value={inputOpml}
            onChange={(e) => setInputOpml(e.target.value)}
            placeholder={'Paste OPML markup here...\ne.g. <opml version="2.0"><body><outline text="Feeds" /></body></opml>'}
        ...
        <div className="lg:col-span-5 flex flex-col">
          ...
          <textarea
            value={outputOpml}
            readOnly
            placeholder="Beautified or minified OPML result will appear here..."
